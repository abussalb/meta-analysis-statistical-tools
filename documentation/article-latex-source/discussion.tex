...
The addition of the three new studies \citep{Strehl2017, Baumeister2016, Bazanova2018} further confirmed the original results. Indeed, the
significance did not change for any outcome: the \gls{se} remained significant for \gls{mprox} raters and
non-significant for \gls{pblind}. Adding three more studies increased the significance of the sensitivity analysis run by
...
Enriquez2017, Vernon2004, Jeunet2018, Arns2009, Cortese2016}. However, to the best of our knowledge, none of these studies has 
...
Two observations were detected as outliers and so removed from the dataset before performing the \gls{saob}: \citeauthor{Bazanova2018}'s individualized \gls{nfb} and 
...
\gls{adhd} \citep{Luan2017}. These large values broke our working hypothesis, so in order to be able to conclude on the results obtained by the \gls{saob}, an outlier 
rejection was implemented.

As expected, the number of sessions was found to be significant, even if it was by only two methods, which was in compliance with existing literature.
For instance, using several univariate regressions without correction for multiple testing \citet{Arns2009}, 
...
insisted that the number of sessions should be carefully chosen in order to avoid "overtraining". The fact that the number of sessions was not 
...
this dataset. However, the two methods that identified this factor both agreed that as expected, the more sessions performed, 
...
The type of \gls{nfb} protocol was not identified by any method, and did not appear to influence the \gls{nfb} results. 
...
Next, it appears that the longer the course of treatment, the less efficient it becomes. This may be explained by the degree of 
...
Some other factors' influence would have been interesting to investigate, such as using personalized \gls{nfb} protocols based on the 
\gls{iapf} \citep{Liu2016}, which seems promising according to \citet{Bazanova2018, Escolano2014}. However, it could not be included in the 
...
The top node splits: on the one hand 46 observations corresponding to \gls{mprox} raters and, on the other, 
...
of the implication in the treatment. This was indeed the case: treatment length was found to be significant but not in the
