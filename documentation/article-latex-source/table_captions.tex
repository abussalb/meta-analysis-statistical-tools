...
\begin{table}[h!]
  ...
  \caption{Comparison between \citet{Cortese2016} results obtained with RevMan \citep{RevMan} and those obtained with the meta-analysis package with our 
	choices$^a$ applied. \glsentryfirstplural{se} (\glspl{se}) and their corresponding p-value (in parenthesis) are presented. 
	With the meta-analysis package, a negative \gls{se} is in favor of \glsfirst{nfb} unlike \citeauthor{Cortese2016}.}
...
\begin{table}[h!]
  ...
  \input{tables/table_factors_analysis_results_summary}
	\begin{tablenotes}
	\begin{footnotesize}
	\item $^a$ Probably Blind Raters, $^b$ Institutional Review Board, $^c$ Sensorimotor Rhythm, $^d$ Slow Cortical Potential, $^e$ Electro-Oculogram.
	\end{footnotesize}
	\end{tablenotes}
