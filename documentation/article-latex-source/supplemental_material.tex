...
the pooled within-group Pearson correlation r (\textit{i.e} the pre-post correlation) was required 
...
\end{equation} 
\noindent An between-\gls{es} is exactly equivalent to a z-score of a standard normal distribution, it is computed as mean pre- to post-treatment 
...
To compute the variance of the between-\gls{es}, the pooled within-group Pearson correlation $\rho$ (\textit{i.e} the pre-post correlation) is required 
\citep{James2013}:
...
\noindent Nevertheless, we choose to use the random effects model, so the weights ($w_{\text{random}_k} = w_k$) associated to this model are different. 
...
\noindent Eventually, the weighted average of the $K$ between-\gls{es} was computed to obtain the \gls{se} as described in 
...
in the linear model thanks to $\ell_1$-norm applied on the coefficients. The coefficients $\hat{\beta}_j$ are obtained by minimizing the term
\begin{equation}
...
as listed in \cref{Table:meta_review_comparison_between_revman_python}, the sign difference is explained by our choice that a negative between-\gls{es}
