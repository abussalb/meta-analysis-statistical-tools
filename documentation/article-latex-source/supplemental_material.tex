...
\end{equation} 
\noindent An between-\gls{es} is equivalent to a z-score of a standard normal distribution. It is computed as mean pre- to post-treatment 
...
\noindent The random-effects model takes into account the differences between the studies, so the weights are equal to the inverse of the sum between the 
...
\noindent Finally, the weighted average of the $K$ between-\gls{es} was computed to obtain the \gls{se} as described in 
