...
\begin{itemize}
  ...
  \item the publication (or subsequently their corresponding author) have to disclose sufficient details about the data
...
The code is made fully available on a GitHub repository \citep{Bussalb2018}, together with all \glspl{rct} raw data we have used
...
As initially suggested by \citeauthor{Cortese2016}, the analysis was ran on two subgroups of studies 
...
were standardized by subtracting the mean and then dividing by the standard deviation (not applied before the decision tree \textcolor{red}{- see later}).
...
These methods are intrinsically different from each other, so we compared their results. For instance, the decision
