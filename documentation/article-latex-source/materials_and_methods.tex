...
Search terms described in Supplemental Material\todo{give reference here} were entered in Pubmed and studies included in previous meta-analysis were identified. Among these studies, those that
...
(active, semi-active or\todo{please check oxford comma throughout the manuscript. } 
...
As initially suggested, we also performed two subgroups analysis with the two choices described above: first, only  with
...
While revisiting the meta-analysis,\todo{Check out your style often you can remove quite a few words in a sentence
  without affecting its meaning but increasing readability; make sure you ask yourself `which words can I remove in this
  sentence'} it became apparent that the studies pooled together where highly heterogeneous 
...
\item \emph{the acquisition quality}: the presence of one or more active electrodes and the \gls{eeg} quality. 
  ...
  poor\todo{define poor. Maybe start with 3 and 2 where definitions are explicit, and finish by ``otherwise a score of
    zero was given for this criteria''} or unknown conditions then the indicator was 1. 
  Besides, if the article did not mention the recording conditions, the factor was set to 1.\todo{not clear/repetition/
    this description has to be re-organised} To get an indicator bigger than 1, the following three  
  points had to be satisfied:\todo{use \emph{describe} environement instead of \emph{itemize}}
\begin{itemize}
  ...
  \href{http://www.eemagine.com/}{Eemagine})\todo{Device Name (Manufacturer, City, Country) for reference to hardware
    and software (add version for the latter)} were preferable or 
  whose reliability is known.\todo{last part of the sentence is vague}
\end{itemize}
\todo{this last line is not item, so preferably to move it out ot itemize env.} If at least one point was checked, the
quality was set to 2. To get an indicator of 3, all these criteria had to be satisfied.\todo{again, some repetition, not
  clear - to be re-organised}
\end{itemize}
...
and its absence 0. All variables were standardized\todo{how?}, except when the decision tree was performed. 
...
The \gls{es} was then considered as a dependent variable to be explained by the factors (the independent variables). The  following three methods, 
...
the number of behavioral scales available in the study.\todo{I would suggest this item is re-wrtiten as it is a little
  confusing in its current state.} 
